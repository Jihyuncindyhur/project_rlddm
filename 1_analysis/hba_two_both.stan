...
data {
  ...
  int<lower=1> T;                    // trial
  int<lower=0> Steps;              // total number of steps
...
parameters {
  ...
  vector[N] tau_pr;                   
  vector[N] b1_pr;    // constant go bias in escape condition                
  vector[N] b2_pr;    // constant go bias in avoid condition
  vector[N] b3_pr;    // drift rate differential action value weight
  vector[N] w1_pr;                    
  vector[N] w2_pr;                    
  vector[N] alpha_pr;                 
  vector[N] omega_pr;               
...
transformed parameters {
  ...
  vector<lower=-20, upper=20>[N] b1;                          // constant go bias in escape condition
  vector<lower=-20, upper=20>[N] b2;                          // constant go bias in avoid condition
  vector<lower=-20, upper=20>[N] b3;                          // drift rate differential action value weight
  vector<lower=0.001, upper=0.999>[N] w1;                     // starting point, escape condition
  vector<lower=0.001, upper=0.999>[N] w2;                     // starting point, avoid condition
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:T) {
      ...
      w = (E[t, i] == 1) ? w1[i] : w2[i];                                               // two starting points 
...
generated quantities {
  ...
  real<lower=-20, upper=20>             mu_b1;          // constant go bias in escape
  real<lower=-20, upper=20>             mu_b2;          // constant go bias in avoid
  ...
  */
  real<lower=-20, upper=20>             mu_b1_sui;            // constant go bias in escape
  real<lower=-20, upper=20>             mu_b1_nosui;          // constant go bias in escape
  real<lower=-20, upper=20>             mu_b2_sui;            // constant go bias in avoid
  real<lower=-20, upper=20>             mu_b2_nosui;          // constant go bias in avoid
  
  real<lower=-20, upper=20>             mu_b3_sui;            // drift rate differential action value weight
  real<lower=-20, upper=20>             mu_b3_nosui;          // drift rate differential action value weight
